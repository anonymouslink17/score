...
class MultilayerGraph:
    def __init__(self, dataset_path=None, flat=False):
        # ****** instance variables ******
        # layers
        ...
        if dataset_path is not None:
            ...
            self.load_dataset(dataset_path, flat=flat)
    ...
    def load_dataset(self, dataset_path, flat=False):
        # open the file
        ...
        try:
            ...
            if flat:
                self.adjacency_list = [[array('i')] for _ in self.nodes_iterator]
            else:
            ...
            # create the empty adjacency list
            if flat:
                self.flat_adjacency_list = [[array('i')] for _ in self.nodes_iterator]
            else:
                self.adjacency_list = [[array('i') for _ in self.layers_iterator] for _ in self.nodes_iterator]
        ...
        self.weights = [[1 for _ in self.layers_iterator] for __ in self.nodes_iterator] 
...
class DirectedMultilayerGraph:
    def __init__(self, dataset_path=None):
        ...
        self.maximum_node = 0
        self.nodes_iterator = range(0)
        self.in_adjacency_list = []
        self.out_adjacency_list = []
    ...
    def load_dataset(self, dataset_path):
        ...
        self.nodes_iterator = range(self.maximum_node + 1)
        # create the empty adjacency list
        self.in_adjacency_list = [[array('i') for _ in self.layers_iterator] for _ in self.nodes_iterator]
        self.out_adjacency_list = [[array('i') for _ in self.layers_iterator] for _ in self.nodes_iterator]
    ...
    def add_edge(self, from_node, to_node, layer):
        ...
        if from_node != to_node:
            # add the edge
            self.out_adjacency_list[from_node][layer].append(to_node)
            self.in_adjacency_list[to_node][layer].append(from_node)
    ...
    # ****** edges ******
    def get_number_of_edges(self, layer=None):
        number_of_in_edges = 0

        for neighbors in self.in_adjacency_list:
            for inner_layer, layer_neighbors in enumerate(neighbors):
                if layer is None:
                    number_of_in_edges += len(layer_neighbors)
                elif layer == inner_layer:
                    number_of_in_edges += len(layer_neighbors)

        return number_of_in_edges

    def get_number_of_edges_layer_by_layer(self):
        number_of_in_edges_layer_by_layer = {}
        number_of_out_edges_layer_by_layer = {}

        for layer in self.layers_iterator:
            number_of_in_edges_layer_by_layer[layer] = sum([len(neighbors[layer]) for neighbors in self.in_adjacency_list])
        
        for layer in self.layers_iterator:
            number_of_out_edges_layer_by_layer[layer] = sum([len(neighbors[layer]) for neighbors in self.out_adjacency_list])

        return number_of_in_edges_layer_by_layer, number_of_out_edges_layer_by_layer
